...
`requires_runtime` suite. This test reads the declared `requires_version` range
(the `agentguides>=…,<…` dev-group constraint in pyproject.toml) and asserts the
...
def test_installed_runtime_satisfies_declared_range() -> None:
    ...
    assert installed in spec, (
        f"installed agentguides {installed} does not satisfy declared requires_version {spec}"
    )
